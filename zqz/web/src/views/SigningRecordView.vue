  ...
  <ContentField>
    <table class="table table-striped table-hover" style="text-align: center;">
      ...
      <tr v-for="record in signRecords" :key="record.id">
        ...
        <td>{{
            record.docUrl === null ? '未开启签约流程' : record.status === 0 ? '待医生确认' : record.status === 1 ? '待医院管理员确认' : record.status === 2 ? '已完成' : ''
          }}
        </td>
        <td>
          <div class="custom-file">
            <div v-if="store.state.user.role === 1">
              <label :for="'customFile_' + record.id" class="custom-file-upload">
                <input type="file" @change="upload(record.id,record.doctorId, $event)" :id="'customFile_' + record.id"
                       style="display: none;">
                上传文件
              </label>
            </div>
            <div v-else>
              <el-button plain @click="goToSignInfo(record.id,record.doctorId)">
                签字确认
              </el-button>
            </div>
          </div>
...
function goToSignInfo(id, doctorId) {
  router.push({name: "sign_info_view", query: {id: id, doctorId: doctorId}});
}
