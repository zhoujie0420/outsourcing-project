...
//确认收货
const updateStatus = useThrottleFn((row) => {
  ElMessageBox.confirm("确认发货?", "警告", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(function () {
    request({
      url: "/api/orderr/updateStatus",
      data: {
        id: row.id,
        statuss: "已发货",
      },
...
<template>
  ...
  <el-card style="margin: 10px">
    <el-table v-loading="loading" :data="pageDataList" border>
